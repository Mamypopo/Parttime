  ...
  <div class="flex flex-col min-h-screen bg-[#F2F5FF]">
    ...
    >
      ...
      >
        ...
        <!-- Admin Profile -->
        <div
          class="absolute bottom-0 left-0 right-0 bg-gradient-to-r from-[#bbe4e1] to-[#B3E5FC] p-8 transition-all duration-300 h-[200px]"
          :class="[isCollapsed ? 'w-16' : 'w-70']"
        >
    ...
    >
      <button
        v-show="isCollapsed && !isTablet && !isMobile"
        @click="toggleSidebar"
    ...
    <nav v-if="isMobile" class="fixed bottom-0 left-0 right-0 bg-white border-t px-4 py-2">
      ...
      >
        ...
        >
          ...
          <router-link
            v-for="item in mobileMainItems.find((i) => i.name === 'ผู้ใช้งาน')?.submenu.items"
            :key="item.path"
            :to="item.path"
      ...
      >
        ...
        >
          ...
          <template
            v-for="item in mobileMainItems.find((i) => i.name === 'เพิ่มเติม')?.submenu.items"
            :key="item.name"
          >
            <router-link
              v-if="!item.action"
